...
Promise.all([
    ...
    // setInterval(async ()=>{ console.log(1)
    //   const a = await checkSingleFrame()
    //   try {
    //     console.log(a)
    //   } catch (error) {
    //     console.log(error)
  ...
  const onPlay =  async ()=> {
    ...
    if (result) { 
        ...
        if(result.box.width > 330){
          ...
          const _x = result.box['x']-result.box['width']/20
          const _y = result.box['y']-result.box['height']/20
          const _width = result.box['width']*1.1
          const _height = result.box['height']*1.1
...
const checkSingleFrame = async ()=>{
  ...
  return new Promise((resolve, reject)=>{
    if(image_result && image_result.box.width > 300){
...
const verify = async (clean_data)=>{
  sessionStorage.setItem('verify_status', true)
  sessionStorage.setItem('img', clean_data.imgData)
  console.log(clean_data.imgData)
